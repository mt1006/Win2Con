...
float glCharW = 0.0f, glCharH = 0.0f;
...
static MSG msg;
static bool charsetLoaded = false, drawCalled = false;
...
void initOpenGlConsole(void)
{
	...
	if (!conhostWindow) { error("The current console process is not \"conhost.exe\"!", "glConsole.c", __LINE__); }
...
void refreshFont(void)
{
	...
	fontW = (int)round((double)conWndW / (double)realConW);
	fontH = (int)round((double)conWndH / (double)realConH);
...
void drawWithOpenGL(GlConsoleChar* output, int w, int h)
{
	...
	setMatrix();
	refreshArrays(w, h);
	glClearColor(12.0 / 255.0, 12.0 / 255.0, 12.0 / 255.0, 1.0);
...
static void createCharacter(char* text, GLuint textureID)
{
	...
	textRect.right = glCharW;
	textRect.bottom = glCharH;
